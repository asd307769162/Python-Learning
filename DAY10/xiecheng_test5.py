...
def get_info(soup, name):
    ...
    for i in range(length):
        ...
        # file = open(name+"评论内容.txt", "a", encoding='utf-8')
        # file.write(result[i])
        ...
        try:
            ...
            cursor = conn.cursor()
            SQL = "insert into Ctrip (username, destination, usercomment, score, time, platform) values ('{0}', '{1}', '{2}', {3}, '{4}', '{5}')".format(username, name, usercomment, score, time, "携程")
            # print(SQL)
            cursor.execute(SQL)
            conn.commit()
            conn.close()
